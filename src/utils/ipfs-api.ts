/**
 * IPFS Daemon HTTP API Client Utility
 *
 ...
 *
 * Used by both the shim (listen mode) and client bridge (forward mode).
 */
...
/**
 * Check if the IPFS daemon is running by pinging /api/v0/id.
 * Returns true if the daemon responds with HTTP 200.
 */
 ...
 * Throws IpfsDaemonNotRunningError if the daemon is not reachable.
 */
...
/**
 * Get the full peer identity information.
 * Throws IpfsDaemonNotRunningError if the daemon is not reachable.
 */
...
/**
 * Check whether Experimental.Libp2pStreamMounting is enabled.
 * Throws Libp2pStreamMountingDisabledError if not enabled.
 * Throws IpfsDaemonNotRunningError if the daemon is not reachable.
 */
...
/**
 * Register a p2p listener: protocol → target address.
 * Kubo endpoint: POST /api/v0/p2p/listen?arg=<protocol>&arg=<target>
 */
export async function p2pListen(
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `p2pListen failed: ${response.status} ${text}`
    );
  }
}

/**
 * Create a p2p forward tunnel: protocol → listen address → target PeerID.
 * Kubo endpoint: POST /api/v0/p2p/forward?arg=<protocol>&arg=<listen>&arg=<target>
 */
export async function p2pForward(
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `p2pForward failed: ${response.status} ${text}`
    );
  }
}

/**
 * Close p2p tunnels. If protocol is provided, close only that protocol.
 * Otherwise close all tunnels.
 * Kubo endpoint: POST /api/v0/p2p/close?all=true  or  ?protocol-id=<proto>
 */
 ...
 * Kubo endpoint: POST /api/v0/p2p/ls?headers=true
 */
